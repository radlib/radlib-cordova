...
function readStartStop(){
	if(document.getElementById("buttonStartStop").innerHTML == "Start Reading"){
		bluetoothScanner.connect(updateTable, unsuccessful, "");
		document.getElementById("buttonStartStop").innerHTML = "Stop Reading";
	}else{
		bluetoothScanner.stopDiscovery(successful, unsuccessful);
...
function BTRFID(){
	bluetoothScanner.connect(updateTable, unsuccessful, "00:14:03:02:03:26");
}
...
//successful callback function. currently used as a success/error dump
function successful(data){
	document.getElementById("status").innerHTML = data;
}

//unsuccessful callback function. currently used as a success/error dump
...
function toggleBT(){
	if(document.getElementById("toggleBT").innerHTML == "Turn Bluetooth On"){
		bluetoothScanner.turnOnBT(successful, unsuccessful);
		document.getElementById("toggleBT").innerHTML = "Turn Bluetooth Off";
	}else{
		bluetoothScanner.turnOffBT(successful, unsuccessful);
